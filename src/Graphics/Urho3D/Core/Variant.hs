...
instance VariantStorable Vector4 where
  setVariant v ptr = liftIO $ with v $ \v' -> [C.exp| void { *$(Variant* ptr) = *$(Vector4* v') } |]
  getVariant ptr = liftIO $ do
    t <- variantType ptr
    case t of
      VariantVector4 -> do
        v <- peek =<< [C.exp| const Vector4* { &$(Variant* ptr)->GetVector4() } |]
        return $ Just v
      _ -> return Nothing

instance VariantStorable Quaternion where
  setVariant v ptr = liftIO $ with v $ \v' -> [C.exp| void { *$(Variant* ptr) = *$(Quaternion* v') } |]
  getVariant ptr = liftIO $ do
    t <- variantType ptr
    case t of
      VariantQuaternion -> do
        v <- peek =<< [C.exp| const Quaternion* { &$(Variant* ptr)->GetQuaternion() } |]
        return $ Just v
      _ -> return Nothing

instance VariantStorable Color where
  setVariant v ptr = liftIO $ with v $ \v' -> [C.exp| void { *$(Variant* ptr) = *$(Color* v') } |]
  getVariant ptr = liftIO $ do
    t <- variantType ptr
    case t of
      VariantColor -> do
        v <- peek =<< [C.exp| const Color* { &$(Variant* ptr)->GetColor() } |]
        return $ Just v
      _ -> return Nothing
...
instance VariantStorable Matrix3x4 where
  setVariant v ptr = liftIO $ with v $ \v' -> [C.exp| void { *$(Variant* ptr) = *$(Matrix3x4* v') } |]
  getVariant ptr = liftIO $ do
    t <- variantType ptr
    case t of
      VariantMatrix3x4 -> do
        v <- peek =<< [C.exp| const Matrix3x4* { &$(Variant* ptr)->GetMatrix3x4() } |]
        return $ Just v
      _ -> return Nothing

instance VariantStorable Matrix4 where
  setVariant v ptr = liftIO $ with v $ \v' -> [C.exp| void { *$(Variant* ptr) = *$(Matrix4* v') } |]
  getVariant ptr = liftIO $ do
    t <- variantType ptr
    case t of
      VariantMatrix4 -> do
...
instance Storable ResourceRefList where
  ...
  poke ptr ResourceRefList {..} =
    with _resourceRefListObjectType $ \_resourceRefListObjectType' ->
      withForeignVector () _resourceRefListObjectNames $ \_resourceRefListObjectNames' ->
        [C.block| void {

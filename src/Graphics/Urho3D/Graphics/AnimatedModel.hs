...
animatedModelGetModelAttr :: (Parent AnimatedModel a, Pointer p a, MonadIO m)
  => p -- ^ Pointer to AnimatedModel or ascentor
  -> m ResourceRef
animatedModelGetModelAttr p = liftIO $ alloca $ \resptr -> do
  let ptr = parentPointer p
  [C.exp| void {
    *($(ResourceRef* resptr)) = $(AnimatedModel* ptr)->GetModelAttr()
    } |]
  peek resptr
...
animatedModelGetBonesEnabledAttr p = liftIO $ do
  ...
  resptr <- [C.exp| VectorVariant* {
    new VectorVariant($(AnimatedModel* ptr)->GetBonesEnabledAttr())
    } |]
...
animatedModelGetAnimationStatesAttr p = liftIO $ do
  let ptr = parentPointer p
  resptr <- [C.exp| VectorVariant* {
    new VectorVariant($(AnimatedModel* ptr)->GetAnimationStatesAttr())
    } |]
  v <- peekForeignVectorAs resptr
  [C.exp| void { delete $(VectorVariant* resptr) }|]
  pure v

...
withObject :: (MonadIO m, MonadMask m, Createable a) => CreationOptions a -> (a -> m b) -> m b
...
withObject' :: (MonadIO m, MonadMask m, Createable a, NFData b) => CreationOptions a -> (a -> m b) -> m b
withObject' opts f = bracket (newObject opts) deleteObject $ \o -> do 
  v <- f o 
  return $!! v 

-- | Ensures that pointer is removed after IO action
deleteAfter :: (MonadIO m, MonadMask m, Createable a) => a -> (a -> m b) -> m b 
...
deleteAfter' :: (MonadIO m, MonadMask m, NFData b, Createable a) => a -> (a -> m b) -> m b 

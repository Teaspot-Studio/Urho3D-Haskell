...
class Createable a where 
  ...
  newObject :: MonadIO m => CreationOptions a -> m (Ptr a)
  -- | Descructs object, after the operation you haven't to use it
  deleteObject :: MonadIO m => Ptr a -> m ()
...
withObject :: (MonadIO m, MonadMask m, Createable a) => CreationOptions a -> (Ptr a -> m b) -> m b
...
withObject' :: (MonadIO m, MonadMask m, Createable a, NFData b) => CreationOptions a -> (Ptr a -> m b) -> m b
withObject' opts f = bracket (newObject opts) deleteObject $ \o -> do 
  v <- f o 
  return $!! v 

-- | Ensures that pointer is removed after IO action
deleteAfter :: (MonadIO m, MonadMask m, Createable a) => Ptr a -> (Ptr a -> m b) -> m b 
...
deleteAfter' :: (MonadIO m, MonadMask m, NFData b, Createable a) => Ptr a -> (Ptr a -> m b) -> m b 

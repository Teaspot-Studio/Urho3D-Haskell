...
-- | Makes PODVector<T> for given type. You need Storable instance for T.
-- Makes following symbols:
-- instance Creatable PODVectorT
-- instance ReadableVector PODVectorT
-- instance WriteableVector PODVectorT
simplePODVector :: String -> DecsQ
simplePODVector elemName = simplePODVector' elemName elemName

-- | Same as 'simpleVector', but you can provide different names for C-side and Haskell-side.
simplePODVector' :: String -> String -> DecsQ
simplePODVector' cElemName elemName = do
  typedef <- C.verbatim $ "typedef " ++ vectorCpp ++ " " ++ vectorT ++ ";"
  createable <- [d|
    instance Creatable (Ptr $vectorType) where
      type CreationOptions (Ptr $vectorType) = ()
  ...
  where
    vectorType = conT $ mkName vectorT
    elemType = conT $ mkName elemName
    vectorT = "PODVector" ++ elemName ++ ""
    vectorCpp = "PODVector<" ++ elemName ++ ">"

-- | Makes internal representation of PODVector<T> for given type
-- Makes following symbols:
-- data PODVectorT
-- podVectorTCntx :: C.Context
simplePODVectorImpl :: String -> DecsQ
simplePODVectorImpl elemName = sequence [
    return $ DataD [] (mkName podVectorT) [] Nothing [] []
  , podVectorTPtrCntx ^:: [t| C.Context |]
  , podVectorTPtrCntx ^= [e| mempty {
        C.ctxTypesTable = Map.fromList [
          (C.TypeName $cTType, $cntxTType)
  ...
  where
    podVectorT = "PODVector" ++ elemName
    podVectorTPtrCntx = "podVector" ++ elemName ++ "Cntx"
    cTType = return $ LitE $ StringL podVectorT
    cntxTType = [e| return $ ConT $ mkName $cTType |]
